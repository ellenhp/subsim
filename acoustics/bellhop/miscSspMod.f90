...
CONTAINS
  ...
  SUBROUTINE EvaluateSSP( cP, cS, rho, Medium, N1, Freq, Task, ENVFile, PRTFile )
    ...
    CASE ( 'P' )  !  monotone PCHIP ACS profile option 
       CALL cPCHIP(   cP, cS, rho, Medium, N1, Task, ENVFile, PRTFile )
  ...
  SUBROUTINE cPCHIP( cP, cS, rho, Medium, N1, Task, ENVFile, PRTFile )
    ...
    DO iSSP = 1, MaxSSP
       ...
       IF ( iSSP > 1 ) THEN
          IF ( SSP%z( iz ) .LE. SSP%z( iz - 1 ) ) THEN
              ...
              CALL ERROUT( PRTFile, 'F', 'ReadSSP', 'The depths in the SSP must be monotone increasing' )
          END IF
       END IF
