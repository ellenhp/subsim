...
CONTAINS
  ...
  SUBROUTINE ReadSzRz( zMin, zMax )
    ...
    IF ( ANY( Pos%Sz( 1 : Pos%NSz ) < zMin ) ) THEN
       ...
       CALL ERROUT( PRTFile, 'W', 'ReadSzRz', 'Source above or too near the top bdry has been moved down' )
    END IF

    IF ( ANY( Pos%Sz( 1 : Pos%NSz ) > zMax ) ) THEN
       WHERE( Pos%Sz > zMax ) Pos%Sz = zMax
       CALL ERROUT( PRTFile, 'W', 'ReadSzRz', 'Source below or too near the bottom bdry has been moved up' ) 
    END IF

    IF ( ANY( Pos%Rz( 1 : Pos%NRz ) < zMin ) ) THEN
       WHERE( Pos%Rz < zMin ) Pos%Rz = zMin
       CALL ERROUT( PRTFile, 'W', 'ReadSzRz', 'Receiver above or too near the top bdry has been moved down' ) 
    END IF

    IF ( ANY( Pos%Rz( 1 : Pos%NRz ) > zMax ) ) THEN
       WHERE( Pos%Rz > zMax ) Pos%Rz = zMax
       CALL ERROUT( PRTFile, 'W', 'ReadSzRz', 'Receiver below or too near the bottom bdry has been moved up' ) 
    END IF
...
!!$    END IF 
  ...
  SUBROUTINE ReadRcvrRanges
    ...
    IF ( .NOT. monotonic( Pos%rr, Pos%NRr ) ) THEN
       CALL ERROUT( PRTFile, 'F', 'ReadRcvrRanges', 'Receiver ranges are not monotonically increasing' )
    END IF 
  ...
  SUBROUTINE ReadRcvrBearings
    ...
    IF ( .NOT. monotonic( Pos%theta, Pos%Ntheta ) ) THEN
       CALL ERROUT( PRTFile, 'F', 'ReadRcvrBearings', 'Receiver bearings are not monotonically increasing' )
    END IF 
  ...
  SUBROUTINE ReadVector( Nx, x, Description, Units )
    ...
    IF ( Nx <= 0 ) CALL ERROUT( PRTFile, 'F', 'ReadVector', 'Number of ' // Description // 'must be positive'  )

    IF ( ALLOCATED( x ) ) DEALLOCATE( x )
    ALLOCATE( x( MAX( 3, Nx ) ), Stat = IAllocStat )
    IF ( IAllocStat /= 0 ) CALL ERROUT( PRTFile, 'F', 'ReadVector', 'Too many ' // Description )
